...
async function requestGroqChat(apiKey, messages, model) {
  ...
  return data && data.choices && data.choices.length && data.choices[0].message && data.choices[0].message.content
...
document.getElementById('talk').addEventListener('click', async function() {
  ...
  for (const modelName of modelsToTry) {
    try {
      const answer = await requestGroqChat(GROQ_API_KEY, messages, modelName);
      if (answer) {
      ...
      lastError = error;
      console.error(`Groq API Fehler mit Modell ${modelName}:`, error);
    }
  }

  appendChatMessage("Ich bekomme gerade keine Antwort von Groq. Prüfe bitte Key/Limit/Modell und versuch es gleich nochmal.");
  if (lastError) {
    console.error("Letzter Groq Fehler:", lastError);
  }
});
